...
- **Destroy gotchas** `gcp-destroy` handles (state-not-config lesson — the relevant `deletion_policy` must already be applied into state, not just merged): (1) the **servicenetworking peering** blocks VPC delete and can't be dropped by terraform — `google_service_networking_connection` has `deletion_policy = "ABANDON"`, and the workflow runs `gcloud compute networks peerings delete servicenetworking-googleapis-com` after Cloud SQL is gone; (2) the **Cloud SQL user** (`myuser`) can't be `DROP ROLE`d because it owns Flyway-created objects — `google_sql_user` has `deletion_policy = "ABANDON"` (instance deletion removes it anyway). Note `-target=google_sql_database_instance.app` also tears down Cloud Run (it depends on the instance), which is why the ordering works.
- **Known teardown limitation — the VPC/subnet may linger after `gcp-destroy`.** Cloud Run's Direct VPC egress leaves a `serverless-ipv4-*` **addressReservation owned by `serverless.googleapis.com`** that blocks the subnet delete (`resourceInUseByAnotherResource`) and **cannot be force-deleted** — `gcloud compute addresses delete` and terraform both refuse ("already being used by //serverless.googleapis.com/…"). GCP releases it **asynchronously** (up to ~an hour after Cloud Run is gone). All *billable* resources (Cloud SQL, Cloud Run, Artifact Registry, secrets) are destroyed before this point, so **the bill is stopped regardless**; the leftover VPC/subnet are free. Once GCP releases the reservation, **re-run `gcp-destroy`** to remove them (the final destroy step goes green then). Don't try to `gcloud compute addresses delete` the reservation — it won't work until GCP frees it.
