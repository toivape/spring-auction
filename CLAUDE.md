...
- **Destroy has three gotchas** `gcp-destroy` handles in sequence (all the state-not-config lesson — the relevant `deletion_policy` must already be applied into state, not just merged): (1) the **servicenetworking peering** blocks VPC delete and can't be dropped by terraform — `google_service_networking_connection` has `deletion_policy = "ABANDON"`, and the workflow runs `gcloud compute networks peerings delete servicenetworking-googleapis-com` after Cloud SQL is gone; (2) the **Cloud SQL user** (`myuser`) can't be `DROP ROLE`d because it owns Flyway-created objects — `google_sql_user` has `deletion_policy = "ABANDON"` (instance deletion removes it anyway); (3) Cloud Run's Direct VPC egress leaves a lingering **`serverless-ipv4-*` address** that blocks the subnet delete — the workflow `gcloud compute addresses delete`s it before the final destroy. Note `-target=google_sql_database_instance.app` also tears down Cloud Run (it depends on the instance), which is why the ordering works.
