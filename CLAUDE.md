...
- Repo **variables**: `GCP_WIF_PROVIDER`, `GCP_DEPLOY_SA`, `GCP_PROJECT_ID`, `GCP_REGION`, `GCP_APP_BASE_URL`, `GCP_AR_REPOSITORY`, `GCP_MAIL_FROM`, `GOOGLE_CLIENT_ID`. **Secrets** are reused across both clouds (`GOOGLE_CLIENT_SECRET`, `MAILJET_*`, `ADMIN_PASSWORD`, `INGESTION_API_KEY`).
